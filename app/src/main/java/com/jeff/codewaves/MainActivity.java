/* TO WHOM IT MAY CONCERN: this file contains code lifted directly from the example provided in the libmuse SDK. */

package com.jeff.codewaves;
...
//					museVersionText.setText(R.string.undefined);
	...
	public void receiveMuseArtifactPacket(final MuseArtifactPacket p, final Muse muse) {
		...
		if (blinkBuffer.isBlink) {
			TextView curr = (TextView) findViewById(R.id.convo_view);
			curr.append(new String("BLINK"));
			BlinkStruct temp = new BlinkStruct(blinkBuffer.isBlink, blinkBuffer.timestamp);
			currSymbol.add(temp);
			blinkBuffer.isBlink = false;
			bcount++;

			if (bcount == 4) {
				parseSymbol(currSymbol);
				bcount = 0;
			}
		}
	}

	private void getBlinkValue(BlinkStruct blinkie, MuseArtifactPacket p) {
		if (blinkie.timestamp < p.getTimestamp() - 200000) {
			blinkie.isBlink = p.getBlink();
			blinkie.timestamp = p.getTimestamp();
		}
	}
